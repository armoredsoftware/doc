...
\frame{\frametitle{Tools for Trust}
\begin{itemize}
  \item $\wrap{X}{\public{Y}}$ --- Wrap $X$ with $\public{Y}$
  	\begin{itemize}
  	...
  	\item Supports chaining keys from a base key
  	\end{itemize}	
\item $\envelope{\public{K}}{D}$ --- Envelop $D$ with $\public{K}$
    \begin{itemize}
...
A \emph{chain of trust} from $X_0$ to $X_n$ establishes that $X_0$ trusts $X_n$

\[\trusts{X_0}{X_1}\wedge\trusts{X_1}\wedge\ldots\wedge\trusts{X_{n-1}}{X_n}\]

$X_0$ is the \emph{root of trust}.  If $X_0$ is trusted, the entire chain is trusted.
}
