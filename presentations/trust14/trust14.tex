...
\frame{\frametitle{Chaining Measurements}
    \begin{columns}[c]
    \column{.53\textwidth}
    \begin{itemize}
    \item Start with a measurer and store that is trusted
    \item Measure software to be launched
    \item Store the measurement
    \item Launch the new software
    \item Repeat until system boot
    \end{itemize}
    \column{.45\textwidth}
    \begin{figure}
...
\frame{\frametitle{What Do We Know?}
  ...
  Assume we know $0|\hash{A}|\hash{B}$ is correct

  \begin{itemize}
  \item $\hash{A}$ and $\hash{B}$ must be correct
    \begin{itemize}
    \item $A$ and $B$ are the correct binaries
    \item $A$ and $B$ include hash and launch functions
    \end{itemize}
  \item Measurement occurred in the right order
    \begin{itemize}
    \item $\hash{(XY)}\neq\hash{(YX)}$
...
\frame{\frametitle{But Why Trust B?}
  
  A chain exists from Trusted Measurer and Trusted Store to B
...
\frame{\frametitle{Trust is a Preorder}
  ...
  \alert{Trusted Boot builds evidence supporting these chains}
}
...
\frame{\frametitle{One Step from Roots of Trust}
  ...
  \begin{columns}[c]
    \column{.53\textwidth}
    \begin{itemize}
    \item Power-on reset, resettable PCRs set to -1
    \item \texttt{SENTER} called
    \item \texttt{SENTER} resets resettable PCRs to 0
...
\frame{\frametitle{Two Steps from Roots of Trust}
  ...
  \begin{columns}[c]
    ...
    \item \texttt{SENTER} invokes the MLE
    \end{itemize}
    \column{.45\textwidth}
    \begin{figure}
...
\frame{\frametitle{Boot the MLE}
  \begin{columns}[c]
    \column{.53\textwidth}
    \begin{itemize}
      \item \texttt{SENTER} starts the MLE 
        \begin{itemize}
        \item \texttt{SENTER} starts the hypervisor
        \item \texttt{SENTER} passes dom0 to hypervisor
        \item hypervisor starts dom0
        \end{itemize}
      \item dom0 constructs the Armored VP
        \begin{itemize}
        \item Measures the vTPM into the TPM
        \item Starts the vTPM
        \item Measures remaining Armored VMs into the vTPM
        \item Starts remaining Armored VMs
        \item Measures Armored application into the vTPM
        \item Starts the Armored application
        \end{itemize}
    \end{itemize}
    \column{.45\textwidth}
    \begin{figure}
...
\frame{\frametitle{Boot the vTPM}
  \begin{columns}[c]
    \column{.53\textwidth}
    \begin{itemize}
      \item \texttt{SENTER} starts the MLE 
...
\frame{\frametitle{Boot the Armor Virtual Platform}
  \begin{columns}[c]
    \column{.53\textwidth}
    \begin{itemize}
    \item \texttt{SENTER} starts the MLE 
