...
\frame{\frametitle{Certifying AIK}
  ...
  \begin{columns}[c]
    \column{.53\textwidth}
    \begin{itemize}
    \item<1-> Target $ID_n$ requests AIK certification from CA
    \item<2-> CA signs AIK
    \item<3-> CA encrypts signed AIK with requester's EK
    \item<4-> CA sends encrypted AIK to requester
    \item<5-> Requester decrypts encrypted AIK
    \item<6-> Requester sends signed AIK to appraiser
    \end{itemize}
    \column{.45\textwidth}
    \begin{figure}
      \only<1>{\includegraphics[width=0.70\textwidth]{figures/cert-pre.pdf}}
      \only<2>{\includegraphics[width=0.70\textwidth]{figures/cert-gen.pdf}}
      \only<3-4>{\includegraphics[width=0.70\textwidth]{figures/cert-crypt.pdf}}
      \only<5-6>{\includegraphics[width=0.70\textwidth]{figures/cert-decrypt.pdf}}
    \end{figure}
  \end{columns}
}
...
\frame{\frametitle{Why Believe AIK Belongs to $ID_n$?}
  ...
  \item CA is trusted to know $ID_n->EK_n$
  ...
  \alert{AIK is an alias for EK used for signing}
}
