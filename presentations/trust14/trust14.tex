...
\frame{\frametitle{Capabilities for Trust}
  \begin{itemize}
  \item \emph{Strong Identification} --- strongly associating an
    identifier---a public key---with the platform
  \item \emph{Reporting Configuration} --- strongly identifying software and
    hardware---hashing and other measurement---running on the platform.
  \end{itemize}
}
...
\frame{\frametitle{Roots of Trust}
  As the name implies, a \emph{root of trust} forms a base for
  transitively building trust.  Roots of trust are trusted implicitly.
...
\frame{\frametitle{Root of Trust for Measurement}
  ...
  \begin{itemize}
  \item Typically a hash function called on an initial code base
  \item In the Intel TXT process the RTM is SENTER
  \item SENTER measures SINIT into PCR 18 in a TPM
  \item SENTER runs securely in the Intel processor
  \end{itemize}
}
